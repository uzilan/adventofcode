...
data class Program(val name: String, val weight: Int? = null, val programsAbove: List<Program>? = null)

private val branches = mutableListOf<Program>()

fun recursiveCircus(input: String): String {
    val rows = input.split("\n")

    rows.forEach { parseRow(it) }

    val branchesWithProgramsAbove = branches.filter { it.programsAbove != null }
    val allLeaves = branchesWithProgramsAbove.flatMap { it.programsAbove!!.toList() }
    val root = branches.find { branch -> allLeaves.none { leaf -> branch.name == leaf.name } }

    return root!!.name
}

private fun parseRow(row: String) {
    val arrow = row.indexOf("->")
    if (arrow > -1) {
        val (name, weight) = parseNameAndWeight(row.substring(0, arrow))
        val programsAbove = parseProgramsAbove(row.substring(arrow + 3))
        addBranch(name, weight, programsAbove)
    } else {
        val (name, weight) = parseNameAndWeight(row)
        addBranch(name, weight)
    }
}

private fun parseNameAndWeight(row: String): Pair<String, Int> {
    val split = row.split(" ")
    val name = split[0]
    val weight = Integer.parseInt(split[1].substring(1, split[1].length - 1))
    return Pair(name, weight)
}

private fun parseProgramsAbove(row: String): List<Program> {
    val split = row.split(", ")
    return split.map { Program(it) }
}

private fun addBranch(name: String, weight: Int? = null, programsAbove: List<Program>? = null) {
    branches.add(Program(name, weight, programsAbove))
}

fun main(args: Array<String>) {
    val input = """ifyzcgi (14)
axjvvur (50)
