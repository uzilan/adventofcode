...
private var registers: MutableMap<String, Int> = mutableMapOf()

fun iHeardYouLikeRegisters(input: String): Int {
    val rows = input.split("\n")
    val splittedRows = rows.map { it.split(" ") }

    registers = splittedRows.map { it[0] to 0 }.toMap().toMutableMap()
    parseRows(splittedRows)
    return registers.values.max()!!
}

private fun parseRows(rows: List<List<String>>) {
    rows.forEach {
        // for example: b inc 5 if a > 1
        val conditionedRegister = registers.get(it[4])
        val conditionedAmount = Integer.parseInt(it[6])
        val operator = it[5]
        val condition = parseCondition(conditionedRegister!!, operator, conditionedAmount)
        if (condition) {
            val amount = Integer.parseInt(it[2])
            val register = it[0]
            val currentAmount = registers[register]
            if (it[1] == "inc") {
                registers.set(register, currentAmount!! + amount)
            } else {
                registers.set(register, currentAmount!! - amount)
            }
        }
...
private fun parseCondition(register: Int, operator: String, amount: Int): Boolean {
...
fun main(args: Array<String>) {
    val input = """smi inc 781 if epx > -2
...
mx dec -934 if w == 4068"""
    ...
    println(iHeardYouLikeRegisters(input))
}